...
module.exports = {
  ...
  initConfig: function(){
    ...
    this.connection = {
      "consumer_key": process.env.CONSUMER_KEY || userConfig.twitter.consumer_key,
      "consumer_secret": process.env.CONSUMER_SECRET || userConfig.twitter.consumer_secret,
      "access_token_key": process.env.ACCESS_TOKEN_KEY || userConfig.twitter.access_token_key,
      "access_token_secret": process.env.ACCESS_TOKEN_SECRET || userConfig.twitter.access_token_secret,
  ...
  saveStream: function(client,writeStream,test) {
  ...
  getCompareFileSize: function(file,maxSize,cb) {
    compareFileSize(file,maxSize,function(ret) {
      return cb(ret);
    });
  },
  testSaveStream: function(writeStream) {
    this.saveStream("test",writeStream);
  }
};
